...
export default function DashBoard() {
  ...
  // ... (Giữ nguyên các State khác không đổi) ...
  const [countOrder, setCountOrder] = useState({
    countTodayOrders: 0, status: "no_change", percent: 0,
  });
  const [revenue, setRevenue] = useState({
    revenueToday: 0, status: "no_change", percent: 0,
  });
  const [countUser, setCountUser] = useState({
    countUser: 0, status: "no_change", percent: 0,
  });
  const [bestSell, setBestSell] = useState("Giò lụa");
  const [dataChart, setDataChart] = useState({
    revenue7days: [], revenue30days: [], revenue90days: [],
  });
  const [recentOrders, setRecentOrders] = useState([
    { orderId: "start", orderCode: "DH2025-000004", status: "unconfirmed", customerName: "Nguyễn Văn a", amount: 0 },
  ]);
  const [warningStock, setWarningStock] = useState([
    { foodId: "start", foodName: "Món", stock: 0, status: "low" },
  ]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        const [
          countRes, revenueRes, userRes, foodRes, recentOrderRes, warningStockRes,
        ] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const loadRevenueChart = async () => {
      try {
        const res = await statistic.getRevenue(Number(range), controller.signal);
  ...
  return (
    <div className="flex flex-col gap-6 pb-6 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto">
        ...
        {/* Revenue Chart: Giữ nguyên styles cũ */}
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col h-[400px] lg:h-[450px]">
